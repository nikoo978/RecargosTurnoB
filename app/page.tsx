...
export default function Page() {
  ...
  function generarComparativo() {
    ...
    for (const p of personal) {
      ...
      ctx.fillStyle = ratio < 0.34 ? "#f2f8f2" : ratio < 0.67 ? "#e0f1e0" : "#cae4ca";
  ...
  return (
    <main className="app">
      <header className="header">
        <h1>RecargosTurnoB</h1>
        <div className="green-line" />
        <div className="sub">Turno B | Recargos y Francos</div>
      </header>
            ...
            <table>
              ...
              <tbody>
                {personal.map((p) => {
                  const s = stats[p] || { guardias: 0, manana: 0, tarde: 0, noche: 0, total: 0, francos: 0, novedades: 0, detalle: [] };
                  return (
                    <tr key={p} onClick={() => setSelectedPerson(p)} style={{ cursor: "pointer", background: selectedPerson === p ? "#e8f5eb" : undefined }}>
